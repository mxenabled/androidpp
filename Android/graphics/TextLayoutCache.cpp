...
#define DEBUG_GLYPHS false

//--------------------------------------------------------------------------------------------------

TextLayoutCache::TextLayoutCache(TextLayoutShaper* shaper) :
        mShaper(shaper),
        mCache(GenerationCache<TextLayoutCacheKey, shared_ptr<TextLayoutValue> >::kUnlimitedCapacity),
...
//    mDebugEnabled = true;
//    CCLOG("Using debug - Debug Enabled = %d", mDebugEnabled);

    mCacheStartTime = systemTime(SYSTEM_TIME_MONOTONIC);

    if (mDebugEnabled) {
        CCLOG("Initialization is done - Start time = %lld", mCacheStartTime);
    }
 ...
 *  Callbacks
 */
void TextLayoutCache::operator()(TextLayoutCacheKey& text, shared_ptr<TextLayoutValue>& desc) {
    size_t totalSizeToDelete = text.getSize() + desc->getSize();
    mSize -= totalSizeToDelete;
    if (mDebugEnabled) {
        CCLOG("Cache value %p deleted, size = %d", desc.get(), totalSizeToDelete);
    }
}
 ...
 * Caching
 */
shared_ptr<TextLayoutValue> TextLayoutCache::getValue(const SkPaint* paint,
            const UChar* text, int start, int count, int contextCount, int dirFlags) {
    AutoMutex _l(mLock);
    nsecs_t startTime = 0;
    if (mDebugEnabled) {
    ...
    shared_ptr<TextLayoutValue> value = mCache.get(key);
    ...
    if (value == NULL) {
        ...
        value = make_shared<TextLayoutValue>(contextCount);
        ...
        if (size <= mMaxSize) {
            ...
            if (mSize + size > mMaxSize) {
                if (mDebugEnabled) {
                    CCLOG("Need to clean some entries for making some room for a new entry");
                }
                while (mSize + size > mMaxSize) {
                    // This will call the callback
                    bool removedOne = mCache.removeOldest();
                    if (!removedOne) {
                        CCLOG("The cache is non-empty but we "
            ...
            if (!putOne) {
                CCLOG("Failed to put an entry into the cache.  "
                        ...
                        start, count, contextCount, String16((char16_t*)(text + start), count).string());
            }
//            if (mDebugEnabled) {
//                nsecs_t totalTime = systemTime(SYSTEM_TIME_MONOTONIC) - startTime;
//                CCLOG("CACHE MISS: Added entry %p "
//                        "with start = %d, count = %d, contextCount = %d, "
//                        "entry size %d bytes, remaining space %d bytes"
//                        " - Compute time %0.6f ms - Put time %0.6f ms - Text = '%s'",
//                        value.getPointer(), start, count, contextCount, size, mMaxSize - mSize,
//                        value->getElapsedTime() * 0.000001f,
//                        (totalTime - value->getElapsedTime()) * 0.000001f,
//                        String16(text + start, count).string());
//            }
        }// else {
//            if (mDebugEnabled) {
//                CCLOG("CACHE MISS: Calculated but not storing entry because it is too big "
//                        "with start = %d, count = %d, contextCount = %d, "
//                        "entry size %d bytes, remaining space %d bytes"
...
//                CCLOG("CACHE HIT #%d with start = %d, count = %d, contextCount = %d"
...
void TextLayoutCache::dumpCacheStats() {
    ...
    CCLOG("------------------------------------------------");
    CCLOG("pid       : %d", getpid());
    CCLOG("running   : %.0f seconds", timeRunningInSec);
    CCLOG("entries   : %zu", cacheSize);
    CCLOG("max size  : %d bytes", mMaxSize);
    CCLOG("used      : %d bytes according to mSize, %zu bytes actual", mSize, bytes);
    CCLOG("remaining : %d bytes or %2.2f percent", mMaxSize - mSize, remainingPercent);
    CCLOG("hits      : %d", mCacheHitCount);
    CCLOG("saved     : %0.6f ms", mNanosecondsSaved * 0.000001f);
    CCLOG("------------------------------------------------");
}
...
#if DEBUG_ADVANCES
    CCLOG("Advances - start = %d, count = %d, contextCount = %d, totalAdvance = %f", start, count,
            ...
            if (bidi) {
...
#if DEBUG_GLYPHS
                String8 test((char16_t*) (chars + start), count);
                CCLOG("******** ComputeValues -- start");
                CCLOG("      -- string = '%s'", test.string());
                CCLOG("      -- start = %d", start);
                CCLOG("      -- count = %d", count);
                CCLOG("      -- contextCount = %d", contextCount);
                CCLOG("      -- bidiReq = %d", bidiReq);
#endif
                ...
                if (U_SUCCESS(status)) {
...
#if DEBUG_GLYPHS
                    CCLOG("      -- dirFlags = %d", dirFlags);
                    CCLOG("      -- paraDir = %d", paraDir);
                    CCLOG("      -- run-count = %d", int(rc));
#endif
                    if (U_SUCCESS(status) && rc == 1) {
                        ...
                        CCLOG("Need to force to single run -- string = '%s',"
                                " status = %d, rc = %d",
                                String8((char16_t*) (chars + start), count).string(), status, int(rc));
                        isRTL = (paraDir == 1);
                        useSingleRun = true;
                    } else {
                        ...
                        for (size_t i = 0; i < size_t(rc); ++i) {
                            ...
                            float runTotalAdvance = 0;
#if DEBUG_GLYPHS
                            CCLOG("Processing Bidi Run = %d -- run-start = %d, run-len = %d, isRTL = %d",
                    ...
                    CCLOG("Cannot set Para");
                    useSingleRun = true;
                    isRTL = (bidiReq = 1) || (bidiReq = UBIDI_DEFAULT_RTL);
                }
                ubidi_close(bidi);
            } else {
                CCLOG("Cannot ubidi_open()");
                useSingleRun = true;
                isRTL = (bidiReq = 1) || (bidiReq = UBIDI_DEFAULT_RTL);
            }
        }
...
#if DEBUG_GLYPHS
        CCLOG("      -- Total returned glyphs-count = %d", outGlyphs->size());
        CCLOG("******** ComputeValues -- end");
#endif
}

static void logGlyphs(HB_ShaperItem shaperItem) {
    CCLOG("         -- glyphs count=%d", shaperItem.num_glyphs);
    for (size_t i = 0; i < shaperItem.num_glyphs; i++) {
        CCLOG("         -- glyph[%d] = %d, offset.x = %0.2f, offset.y = %0.2f", i,
                shaperItem.glyphs[i],
...
#if DEBUG_GLYPHS
            CCLOG("The BiDi run '%s' is containing a Diacritic at position %d",
                    String8((char16_t*) chars, count).string(), int(i));
#endif
            ssize_t j = i - 1;
            for (; j >= 0;  --j) {
                UChar ch2 = chars[j];
...
#if DEBUG_GLYPHS
            CCLOG("Found main code point at index %d", int(j));
#endif
...
#if DEBUG_GLYPHS
            CCLOG("Rewriting codepoint '%d' to '%d' at position %d",
...
#if DEBUG_GLYPHS
    if (useNormalizedString) {
        CCLOG("Will use normalized string '%s', length = %d",
                    String8((char16_t*) mNormalizedString.getTerminatedBuffer(),
                            mNormalizedString.length()).string(),
                    mNormalizedString.length());
    } else {
        CCLOG("Normalization is not needed or cannot be done, using initial string");
    }
#endif

    assert(!useNormalizedString || mNormalizedString.length() == count);

    // Set the string properties
    mShaperItem.string = useNormalizedString ? mNormalizedString.getBuffer() : chars;
...
#if DEBUG_GLYPHS
        CCLOG("-------- Start of Script Run --------");
        CCLOG("Shaping Script Run with");
        CCLOG("         -- isRTL = %d", isRTL);
        CCLOG("         -- HB script = %d", mShaperItem.item.script);
        CCLOG("         -- startFontRun = %d", int(startScriptRun));
        CCLOG("         -- endFontRun = %d", int(endScriptRun));
        CCLOG("         -- countFontRun = %d", countScriptRun);
        CCLOG("         -- run = '%s'", String8((char16_t*) (chars + startScriptRun), countScriptRun).string());
        CCLOG("         -- string = '%s'", String8((char16_t*) chars, count).string());
#endif
...
#if DEBUG_GLYPHS
        CCLOG("Got from Harfbuzz");
        CCLOG("         -- glyphBaseCount = %d", glyphBaseCount);
        CCLOG("         -- num_glypth = %d", mShaperItem.num_glyphs);
        CCLOG("         -- kerning_applied = %d", mShaperItem.kerning_applied);
        CCLOG("         -- isDevKernText = %d", paint->isDevKernText());
...
#if DEBUG_GLYPHS
            CCLOG("Advances array is empty or num_glypth = 0");
#endif
            continue;
        }

#if DEBUG_GLYPHS
        CCLOG("Returned logclusters");
        for (size_t i = 0; i < mShaperItem.num_glyphs; i++) {
            CCLOG("         -- lc[%d] = %d, hb-adv[%d] = %0.2f", i, mShaperItem.log_clusters[i],
                    i, HBFixedToFloat(mShaperItem.advances[i]));
        }
#endif
        // Get Advances and their total
        float currentAdvance = HBFixedToFloat(mShaperItem.advances[mShaperItem.log_clusters[0]]);
        float totalFontRunAdvance = currentAdvance;
...
#if DEBUG_ADVANCES
        CCLOG("Returned advances");
        for (size_t i = 0; i < countScriptRun; i++) {
            CCLOG("         -- hb-adv[%d] = %0.2f, log_clusters = %d, total = %0.2f", i,
...
#if DEBUG_GLYPHS
            CCLOG("Returned script run glyphs -- count = %d", countGlyphs);
#endif
            for (size_t i = 0; i < countGlyphs; i++) {
                UChar glyph = glyphBaseCount +
                        (UChar) mShaperItem.glyphs[(!isRTL) ? i : countGlyphs - 1 - i];
#if DEBUG_GLYPHS
                CCLOG("         -- glyph[%d] = %d", i, glyph);
#endif
                outGlyphs->add(glyph);
            }
        }
...
SkTypeface* TextLayoutShaper::typefaceForUnichar(const SkPaint* paint, SkTypeface* typeface,
    ...
    case HB_Script_Hebrew:
        if (typeface) {
...
#if DEBUG_GLYPHS
                CCLOG("Using Hebrew Bold/BoldItalic Typeface");
#endif
                break;
...
#if DEBUG_GLYPHS
                CCLOG("Using Hebrew Regular/Italic Typeface");
#endif
                break;
            }
        } else {
            typeface = getCachedTypeface(&mHebrewRegularTypeface, TYPE_FACE_HEBREW_REGULAR);
#if DEBUG_GLYPHS
            CCLOG("Using Hebrew Regular Typeface");
#endif
        }
        break;
...
#if DEBUG_GLYPHS
       CCLOG("Using Devanagari Regular Typeface");
#endif
        break;
...
#if DEBUG_GLYPHS
                CCLOG("Using Tamil Bold Typeface");
#endif
                break;
...
#if DEBUG_GLYPHS
                CCLOG("Using Tamil Regular Typeface");
#endif
                break;
            }
        } else {
...
#if DEBUG_GLYPHS
    CCLOG("Run typeface = %p, uniqueID = %d, hb_face = %p",
...
#if DEBUG_GLYPHS
    CCLOG("Creating Glyph Arrays with size = %d", size);
#endif
...
SkTypeface* TextLayoutShaper::getCachedTypeface(SkTypeface** typeface, const char path[]) {
    if (!*typeface) {
        *typeface = SkTypeface::CreateFromFile(path);
        ...
        if (!*typeface) {
#if DEBUG_GLYPHS
        CCLOG("Font path '%s' is not valid, will use default font", path);
#endif
            return mDefaultTypeface;
        }
        (*typeface)->ref();
#if DEBUG_GLYPHS
        CCLOG("Created SkTypeface from file '%s' with uniqueID = %d", path, (*typeface)->uniqueID());
#endif
    }
    return *typeface;
}
...
HB_Face TextLayoutShaper::getCachedHBFace(SkTypeface* typeface) {
    ...
    if (face) {
#if DEBUG_GLYPHS
        CCLOG("Created HB_NewFace %p from paint typeface = %p", face, typeface);
#endif
...
shared_ptr<TextLayoutValue> TextLayoutEngine::getValue(const SkPaint* paint, const UChar* text,
        int start, int count, int contextCount, int dirFlags) {
    shared_ptr<TextLayoutValue> value;
#if USE_TEXT_LAYOUT_CACHE
    value = mTextLayoutCache->getValue(paint, text, start, count,
            contextCount, dirFlags);
    if (value == NULL) {
        CCLOG("Cannot get TextLayoutCache value for text = '%s'",
                String16((char16_t*) (text + start), count).string());
    }
#else
...
void TextLayoutEngine::purgeCaches() {
    ...
    mShaper->purgeCaches();
#if DEBUG_GLYPHS
    CCLOG("Purged TextLayoutEngine caches");
#endif
#endif
}
