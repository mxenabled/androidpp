...
public:
	...
	bool postDelayed(std::function<void ()> function, uint32_t delay);
	bool postAtTime(std::function<void ()> function, uint64_t execTimestamp);
...
private:
	...
	sp<Message> getPostMessage(std::function<void ()> function);

	void handleCallback(const sp<Message>& message) {
		message->mCallback->run();
	}
    
    void handleFunction(const sp<Message>& message) {
        message->mFunction();
    }
