...
bool Handler::post(std::function<void ()> function) {
    if (function) {
        const sp<Message>& message = getPostMessage(function);
        return sendMessage(message);
    } else {
        return false;
    }
...
bool Handler::postDelayed(std::function<void ()> function, uint32_t delay) {
    if (function) {
        const sp<Message>& message = getPostMessage(function);
        return sendMessageDelayed(message, delay);
    } else {
...
bool Handler::postAtTime(std::function<void ()> function, uint64_t execTimestamp) {
    if (function) {
        const sp<Message>& message = getPostMessage(function);
        return sendMessageAtTime(message, execTimestamp);
    } else {
...
sp<Message> Handler::getPostMessage(std::function<void ()> function) {
	const sp<Message> message = Message::obtain();
	message->mFunction = function;
	return message;
}
