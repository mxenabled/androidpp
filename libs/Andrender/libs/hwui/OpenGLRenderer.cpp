...
void OpenGLRenderer::resume() {
    mindroid::sp<Snapshot> snapshot = (mSnapshot != NULL) ? mSnapshot : mFirstSnapshot;
...
bool OpenGLRenderer::restoreSnapshot() {
    ...
    mindroid::sp<Snapshot> current = mSnapshot;
    mindroid::sp<Snapshot> previous = mSnapshot->previous;
 ...
 */
bool OpenGLRenderer::createLayer(mindroid::sp<Snapshot> snapshot, float left, float top,
...
bool OpenGLRenderer::createFboLayer(Layer* layer, Rect& bounds, mindroid::sp<Snapshot> snapshot,
        GLuint previousFbo) {
 ...
 */
void OpenGLRenderer::composeLayer(mindroid::sp<Snapshot> current, mindroid::sp<Snapshot> previous) {
    if (!current->layer) {
...
status_t OpenGLRenderer::drawLines(float* points, int count, SkPaint* paint) {
    ...
    const int wVerticesCount = verticesCount;
    AAVertex* wLines = new AAVertex[wVerticesCount];
...
//#if DEBUG_GLYPHS
//    CCLOG("OpenGLRenderer drawText() with FontID=%d", SkTypeface::UniqueID(paint->getTypeface()));
//#endif
