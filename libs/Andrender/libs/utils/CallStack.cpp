...
bool CallStack::operator < (const CallStack& rhs) const {
    ...
    return false;//memcmp(mStack, rhs.mStack, mCount * sizeof(backtrace_frame_t)) < 0;
}
...
const void* CallStack::operator [] (int index) const {
    if (index >= int(mCount))
        return 0;
    return 0;//reinterpret_cast<const void*>(mStack[index].absolute_pc);
}
...
void CallStack::update(int32_t ignoreDepth, int32_t maxDepth) {
    ...
    ssize_t count = 0;//unwind_backtrace(mStack, ignoreDepth + 1, maxDepth);
