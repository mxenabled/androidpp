...
void BlobCache::set(const void* key, size_t keySize, const void* value,
    ...
    mindroid::sp<Blob> dummyKey(new Blob(key, keySize, false));
    ...
    while (true) {
        ...
        if (index < 0) {
            ...
            mindroid::sp<Blob> keyBlob(new Blob(key, keySize, true));
            mindroid::sp<Blob> valueBlob(new Blob(value, valueSize, true));
            ...
            mindroid::sp<Blob> valueBlob(new Blob(value, valueSize, true));
            mindroid::sp<Blob> oldValueBlob(mCacheEntries[index].getValue());
...
size_t BlobCache::get(const void* key, size_t keySize, void* value,
    ...
    // enough.
    mindroid::sp<Blob> valueBlob(mCacheEntries[index].getValue());
...
size_t BlobCache::getFlattenedSize() const {
    ...
    for (size_t i = 0; i < mCacheEntries.size(); i++) {
        ...
        mindroid::sp<Blob> keyBlob = e.getKey();
        mindroid::sp<Blob> valueBlob = e.getValue();
        size = align4(size);
...
BlobCache::CacheEntry::CacheEntry(const mindroid::sp<Blob>& key, const mindroid::sp<Blob>& value):
        mKey(key),
        mValue(value) {
}
...
mindroid::sp<BlobCache::Blob> BlobCache::CacheEntry::getKey() const {
    return mKey;
}

mindroid::sp<BlobCache::Blob> BlobCache::CacheEntry::getValue() const {
    return mValue;
}

void BlobCache::CacheEntry::setValue(const mindroid::sp<Blob>& value) {
    mValue = value;
}
