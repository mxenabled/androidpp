...
namespace android {
    ...
    struct BufferedTextOutput::BufferState : public mindroid::Ref
    {
    ...
    {
        Vector<mindroid::sp<BufferedTextOutput::BufferState> > states;
    };
...
//        return ts;
    ...
    , mIndex(allocBufferIndex())
    {
        ...
        if (mGlobalState) mGlobalState->incStrongRef(this);
    }
    
    BufferedTextOutput::~BufferedTextOutput()
    {
        if (mGlobalState) mGlobalState->decStrongRef(this);
    ...
    BufferedTextOutput::BufferState* BufferedTextOutput::getBuffer() const
    {
        if ((mFlags&MULTITHREADED) != 0) {
            ...
            if (ts) {
                ...
                BufferState* bs = ts->states[mIndex].getPointer();
                if (bs != NULL && bs->seq == mSeq) return bs;
                
                ts->states.editItemAt(mIndex) = new BufferState(mIndex);
                bs = ts->states[mIndex].getPointer();
