...
namespace android {
    ...
    template<typename EntryKey, typename EntryValue>
    struct Entry: public mindroid::LightweightRef<Entry<EntryKey, EntryValue> > {
        ...
        mindroid::sp<Entry<EntryKey, EntryValue> > parent; // next older entry
        mindroid::sp<Entry<EntryKey, EntryValue> > child;  // next younger entry
    }; // struct Entry
    ...
    private:
        KeyedVector<K, mindroid::sp<Entry<K, V> > > mCache;
        uint32_t mMaxCapacity;
        
        OnEntryRemoved<K, V>* mListener;
        
        mindroid::sp<Entry<K, V> > mOldest;
        mindroid::sp<Entry<K, V> > mYoungest;
        
        void attachToCache(const mindroid::sp<Entry<K, V> >& entry);
        void detachFromCache(const mindroid::sp<Entry<K, V> >& entry);
    ...
    template<typename K, typename V>
    size_t GenerationCache<K, V>::size() const {
        return mCache.size();
    }
    ...
    void GenerationCache<K, V>::clear() {
        if (mListener) {
            for (uint32_t i = 0; i < mCache.size(); i++) {
                mindroid::sp<Entry<K, V> > entry = mCache.valueAt(i);
                if (mListener) {
    ...
    const V& GenerationCache<K, V>::get(const K& key) {
        ...
        if (index >= 0) {
            const mindroid::sp<Entry<K, V> >& entry = mCache.valueAt(index);
    ...
    bool GenerationCache<K, V>::put(const K& key, const V& value) {
        ...
        ssize_t index = mCache.indexOfKey(key);
        if (index < 0) {
            mindroid::sp<Entry<K, V> > entry = new Entry<K, V>(key, value);
    ...
    void GenerationCache<K, V>::removeAt(ssize_t index) {
        mindroid::sp<Entry<K, V> > entry = mCache.valueAt(index);
        if (mListener) {
            (*mListener)(entry->key, entry->value);
        }
    ...
    template<typename K, typename V>
    void GenerationCache<K, V>::attachToCache(const mindroid::sp<Entry<K, V> >& entry) {
        if (!mYoungest.getPointer()) {
    ...
    template<typename K, typename V>
    void GenerationCache<K, V>::detachFromCache(const mindroid::sp<Entry<K, V> >& entry) {
        if (entry->parent.getPointer()) {
            entry->parent->child = entry->child;
        } else {
            mOldest = entry->child;
        }
        
        if (entry->child.getPointer()) {
